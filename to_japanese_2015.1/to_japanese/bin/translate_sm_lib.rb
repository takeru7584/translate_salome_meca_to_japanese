...
module TranslateSMLIB
	...
	VERSION_NUM_STR = "0.17a"
	VERSION_OPTION_STR = "ALPHA (DEVELOPMENT)"
	DEFAULT_DIC_FILE_PATH = "#{File.dirname(__FILE__)}/../dic/org/summary_translate_word.dic"
	DEFAULT_CROWDIN_DIC_FILE_PATH = "#{File.dirname(__FILE__)}/../dic/org/summary_translate_word_from_crowdin.dic"
	DEFAULT_EFICAS_DIC_FILE_PATH = "#{File.dirname(__FILE__)}/../dic/org/summary_translate_word_for_eficas.dic"
	...
	def init()
		...
		@disp_help = false
		@dic_file = "#{DEFAULT_DIC_FILE_PATH}"
		@crowdin_dic_file = "#{DEFAULT_CROWDIN_DIC_FILE_PATH}"
		@eficas_dic_file = "#{DEFAULT_EFICAS_DIC_FILE_PATH}"
		@udic_file = "#{DEFAULT_USER_DIC_FILE_PATH}"
		@log_level = 1
		@log_file = nil
		@debug_mode = false
	...
	def crowdin_dic_file(oIO=STDERR)
		return @crowdin_dic_file
	end

	def eficas_dic_file(oIO=STDERR)
		return @eficas_dic_file
	end
...
#STDERR.puts sFilePath
	...
	def register_dic_from_dat(coDicSummaries=TranslateSummaries.new(), sCheckFilePath)
		...
		# tsファイル名が *_fr.tsで終わっていなければcoDicSummariesを返す
		#return coDicSummaries unless sTSFileName =~ /(.+?_msg_fr).ts$/
		return coDicSummaries unless sTSFileName =~ /(.+?_fr).ts$/
			  # tsファイルのベースファイル名を取得
...
#STDERR.puts "Check4(#{sOutputFilePath})"
		...
		sSummariesKey = "#{sSummariesKey.gsub("_#{sFromLang}","_fr")}"
...
#STDERR.puts hDatabase
	...
	def qm_to_ts_exec(sQMFilePath)
		...
		sMsg = "sQMFilePath = <#{sQMFilePath}(#{sQMFilePath.class})>"
		err_print(ERROR_LEVEL_03, sMsg, STDERR)
		sTSFilePath = sQMFilePath.gsub(/\.qm$/, ".ts")

		# sQMFilePath エラーチェック
		#return sTSFilePath unless sQMFilePath =~ /_msg_(.+?)\.qm$/
		return sTSFilePath unless sQMFilePath =~ /_(\w{2})\.qm$/
...
class TranslateKeyword
	...
	def update(aFactor=["","","",""])
		unless aFactor[3].chomp.strip.size == 0 then
...
#		next unless sFilePath =~ /_msg_\w\w.ts$/
		...
		next unless sFilePath =~ /_\w\w.qm$/
